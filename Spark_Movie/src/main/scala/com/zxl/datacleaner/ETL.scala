...
object ETL {
  def main(args: Array[String]) {
    ...
    val clusterMasterURL = "spark://hadoop-maste:7077"
    ...
    val links = sc.textFile("hdfs://hadoop-maste:9000/movie/data/links.csv", minPartitions).filter { !_.endsWith(",") } //e
      .map(_.split(",")).map(x => Links(x(0).trim.toInt, x(1).trim.toInt, x(2).trim().toInt)).toDF()

    val movies = sc.textFile("hdfs://hadoop-maste:9000/movie/data/movies.csv", minPartitions).filter { !_.endsWith(",") }
      .map(_.split(",")).map(x => Movies(x(0).trim().toInt, x(1).trim(), x(2).trim())).toDF()

    val ratings = sc.textFile("hdfs://hadoop-maste:9000/movie/data/ratings.txt", minPartitions).filter { !_.endsWith(",") }
      .map(_.split(",")).map(x => Ratings(x(0).trim().toInt, x(1).trim().toInt, x(2).trim().toDouble, x(3).trim().toInt)).toDF()

    val tags = sc.textFile("hdfs://hadoop-maste:9000/movie/data/tags.txt", minPartitions).filter { !_.endsWith(",") }.map(x=>rebuild(x)).map(_.split(",")).map(x => Tags(x(0).trim().toInt, x(1).trim().toInt, x(2).trim(), x(3).trim().toInt)).toDF()
